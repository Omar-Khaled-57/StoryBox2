  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        const unlistenMobileScan = listen("trigger-mobile-scan", async (_event) => {
            ...
            // Detect platform — try iOS photos plugin first, fallback to Android medialibrary
            const isIOS = navigator.userAgent.includes("iPhone") || navigator.userAgent.includes("iPad") || navigator.userAgent.includes("iOS");

            if (isIOS) {
                try {
                    const ios = await import("@gbyte/tauri-plugin-ios-photos");
                    const status = await ios.requestPhotosAuth();
                    if (status !== ios.PhotosAuthorizationStatus.authorized && status !== ios.PhotosAuthorizationStatus.limited) {
                        console.warn("[App] iOS photo library permission denied:", status);
                        return;
                    }

                    const albums = await ios.requestAlbums({
                        with: ios.PHAssetCollectionType.smartAlbum,
                        subtype: ios.PHAssetCollectionSubtype.smartAlbumUserLibrary,
                    });

                    const userAlbum = albums.find(() => true); // first album is the user library
                    if (!userAlbum) {
                        console.warn("[App] No user album found on iOS");
                        return;
                    }

                    const medias = await ios.requestAlbumMedias({
                        id: userAlbum.id,
                        height: 1080,
                        width: 1080,
                        quality: 80,
                    ...
                    const items = medias || [];
                    console.log(`[App] iOS photo library found ${items.length} images`);

                    setActivities(prev => [
                        ...prev.filter(a => a.id !== "indexing"),
                        { id: "indexing", label: `Indexing iOS Photos (${items.length})`, progress: 0 },
                    ]);
                    ...
                    for (let i = 0; i < items.length; i += CHUNK_SIZE) {
                        ...
                        await Promise.all(chunk.map((item, idx) =>
                            invoke("index_ios_image_data", {
                                data: item.data || "",
                                fileName: `ios_${item.id}_${idx}.jpg`,
                            })
                        ));
                        setActivities(prev => prev.map(a =>
                            a.id === "indexing" ? { ...a, progress: Math.min((i / items.length) * 100, 99) } : a
                        ));
                    }

                    setActivities(prev => prev.map(a =>
                        a.id === "indexing" ? { ...a, label: "iOS Scan Complete", progress: 100 } : a
                    ));
                    setTimeout(() => setActivities(prev => prev.filter(a => a.id !== "indexing")), 5000);
                    loadStories();
                } catch (e) {
                    console.error("[App] iOS scan failed:", e);
                }
                return;
            }
            ...
            try {
                ...
                console.error("[App] Android scan failed:", e);
            }
        });
